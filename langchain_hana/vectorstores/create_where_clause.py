...
class FilterOperand:
    """
    Represents a filter operand with type information for validation
    and error messages.
    """
...
def _determine_filter_operands(operator: str, operands: Any) -> list[FilterOperand]:
    ...
    if not isinstance(operands, (list, tuple)):
        raise ValueError(
            f"Operator {operator} expects list/tuple of operands, but got {operands}"
        )
    if len(operands) == 0:
...
def _determine_single_filter_operand(operator: str, operands: Any) -> FilterOperand:
    """
    Check that operands is a single value (not list/tuple)
    and return FilterOperand.
    """
    if isinstance(operands, (list, tuple)):
        raise ValueError(
            f"Operator {operator} expects a single operand, "
            f"but got {type(operands).__name__}: {operands}"
        )
    try:
...
def _sql_serialize_logical_clauses(sql_operator: str, sql_clauses: list[str]) -> str:
...
class CreateWhereClause:
    ...
    def _create_where_clause(self, filter: dict) -> Tuple[str, tuple[Any, ...]]:
        if not filter:
        ...
        for key, value in filter.items():
            if key.startswith("$"):
                ...
                if len(value) != 1:
                    raise ValueError(
                        f"Filter expects a single 'operator: operands' entry, "
                        f"but got {value}"
                    )
                operator, operands = list(value.items())[0]
                ret_sql_clause, ret_query_tuple = self._sql_serialize_column_operation(
                ...
                ret_sql_clause = (
                    f"{self._create_selector(key)} = {sql_operand.placeholder}"
                )
    ...
    def _sql_serialize_column_operation(
        ...
        if operator == "$contains":
            ...
            if operand.the_type != "str" or not operand.value:
                raise ValueError(
                    f"Operator $contains expects a non-empty string operand, "
                    f"but got {operand!r}"
                )
            sql_operand = SqlOperand(operand)
            statement = (
                f'SCORE({sql_operand.placeholder} IN ("{column}" '
                f"EXACT SEARCH MODE 'text')) > 0"
            )
        ...
        if operator == "$like":
            ...
            if operand.the_type != "str":
                raise ValueError(
                    f"Operator $like expects a string operand, but got {operand}"
                )
            sql_operand = SqlOperand(operand)
        ...
        if operator == "$between":
            ...
            if len(filter_operands) != 2:
                raise ValueError(
                    f"Operator $between expects 2 operands, but got {filter_operands}"
                )
            from_operand, to_operand = filter_operands
            if from_operand.the_type != to_operand.the_type:
                raise ValueError(
                    f"Operator $between expects operands of the same type, "
                    f"but got {filter_operands}"
                )
            if from_operand.the_type not in ("int", "float", "str", "date"):
                raise ValueError(
                    f"Operator $between expects operand types "
                    f"(int, float, str, date), but got {filter_operands}"
                )
        ...
        if operator in ("$in", "$nin"):
            ...
            for op in filter_operands:
                if op.the_type != filter_operands[0].the_type:
                    raise ValueError(
                        f"Operator {operator} expects operands of the same type, "
                        f"but got {operands}"
                    )
            ...
            sql_values = tuple(sql_operand.value for sql_operand in sql_operands)
        ...
        if operator in ("$gt", "$gte", "$lt", "$lte"):
            ...
            if operand.the_type not in ("int", "float", "str", "date"):
                raise ValueError(
                    f"Operator {operator} expects operand of type "
                    f"int/float/str/date, but got {operand}"
                )
    ...
    def _sql_serialize_logical_operation(
        ...
        if operator in ("$and", "$or"):
            ...
            return _sql_serialize_logical_clauses(
                sql_operator, sql_clauses
            ), query_tuple
