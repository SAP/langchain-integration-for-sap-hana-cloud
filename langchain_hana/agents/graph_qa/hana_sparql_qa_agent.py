...
class HanaSparqlQAAgent:
    ...
    def __init__(self, graph: HanaRdfGraph, model: BaseLanguageModel, tools=None, middleware=None, system_prompt: str = None, include_default_tools: bool = True, include_default_middleware: bool = True):
        self.graph = graph
        self.ontology = self.graph.get_ontology().serialize(format="turtle")
        self.system_prompt = system_prompt or SYSTEM_PROMPT.format(self.graph.graph_uri)
        self.include_default_tools = include_default_tools
        self.include_default_middleware = include_default_middleware
        ...
        if self.include_default_middleware:
    ...
    def create_agent(cls, graph: HanaRdfGraph, model: BaseLanguageModel, tools=None, system_prompt: str = None, middleware=None, **kwargs):
        ...
        instance = cls(graph, model, system_prompt)
        tool_node = ToolNode(instance.tools, handle_tool_errors=True)
        return create_agent(model, tools=tool_node, system_prompt=instance.system_prompt, middleware=middleware, **kwargs)
