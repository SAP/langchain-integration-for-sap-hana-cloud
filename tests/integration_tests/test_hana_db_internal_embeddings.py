...
def is_internal_embedding_available(connection, embedding) -> bool:
    ...
    if not embedding.remote_source:
        ...
        sql_str = (
            f"""VECTOR_EMBEDDING('test', 'QUERY', ?, "{embedding.remote_source}")"""
        )
    try:
        cur = connection.cursor()
        # Test the VECTOR_EMBEDDING function by executing a simple query
        cur.execute(
            ("SELECT TO_NVARCHAR(" f"{sql_str})" "FROM sys.DUMMY;"),
            sql_params,
        )
        cur.fetchall()  # Ensure the query executes successfully
        return True
    except Exception as e:
        print(f"Error checking internal embedding availability: {e}")  # noqa: T201
        return False
    finally:
        cur.close()
...
def test_hanavector_add_texts_map_merge_preserves_order(vectorDB) -> None:
    ...
    # Therefore we use an approximate approach to assert the insertion order
    # is preserved
